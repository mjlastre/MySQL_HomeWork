use sakila;
# --1a. Display the first and last names of all actors from the table act
...
ALTER TABLE actor
ADD COLUMN description blob(2) AFTER last_name;
...
SELECT last_name, COUNT(*) AS 'Total Number of Actors' 
...
SELECT fil.title AS 'Film Title', COUNT(fa.actor_id) AS `Total Number of Actors`
FROM film_actor fa
INNER JOIN film fil 
...
WHERE film.film_id = inventory.film_id) AS 'Total Number of Copies'
...
SELECT film.title, COUNT(rental_id) AS 'Number Times Rented'
FROM rental r JOIN inventory i ON (r.inventory_id = i.inventory_id) JOIN film film
ON (i.film_id = film.film_id) GROUP BY film.title
ORDER BY 'Times Rented' DESC;

#7f. Write a query to display how much business, in dollars, each store brought in.
SELECT s.store_id, SUM(amount) AS 'Revenue' FROM payment p
...
#8c. You find that you no longer need the view top_five_genres. Write a query to delete
